...
- `hidden` 列が `TRUE` の行は収録するが、`hidden: true` として出力する（→ [非表示タレント](#非表示タレントhidden)）
  ...
  "talents": [
    {
      ...
      "hidden": false,           // ゲーム内で露出させないタレント（→「非表示タレント」節）
                                  //   スプレッドシートの hidden 列が TRUE の行のみ true
      ...
      "givenKana": "ヒカリ",      // 名カナ
      "nicknames": ["ひかりん"], // あだ名の配列（0個以上。スプレッドシートの改行区切りを分割）
...
| 非表示制御 | なし | `hidden` | 退学等で露出させないタレントをデータ側で制御 |

### 非表示タレント（hidden）

強制退学等の理由でゲーム内に露出させないタレントを `hidden: true` で制御する。スプレッドシートの `hidden` 列（TRUE / 空欄）を同期時に取り込む。

- **`dropout` 列とは別物**。退学した事実（`dropout`）と、ゲーム内で露出させるか（`hidden`）は独立して扱う。退学者でも `hidden` が空欄なら通常どおり表示される
- 行自体は talents.json に残す。スケルトンパズルのデータ（`public/data/puzzles/*.json`）が `talentId` で talents.json を参照しており、行を削除するとパズルが完成不可能になるため

#### 参照ルール

`useTalents()` は**既定で `hidden: true` を除外**する。新しい画面を素直に実装すれば自動的に非表示側へ倒れる。

| 用途 | 呼び出し | 挙動 |
|------|---------|------|
| クイズ・診断・図鑑・部屋・セリフ等すべて | `useTalents()` | hidden を除外 |
| スケルトンパズル | `useTalents(true)` | hidden を含む（パズル成立のため例外） |

パズル側の例外は、既存パズルデータに非表示タレントが含まれている間の暫定措置。パズルを作り直して該当タレントを除いた後は `useTalents()` に戻してよい。
...
- 選択肢テキストが `#rrggbb` / `#rgb` 形式のカラーコードのみの場合、選択肢ボタンに**その色の見本（スウォッチ）**を表示する
  - 判定は `src/shared/utils/colorCode.ts` の `parseColorCode`
  - スウォッチ＋カラーコード文字列を横並びで表示（文字列も残すことで、色が近い選択肢を区別できるようにする）
  - 正解/不正解時のボタン背景（緑/赤）はそのまま適用し、スウォッチのみ元の色を保つ
- 用途例: ペンライトカラーの問題（`answers` に候補のカラーコードを並べる）
...
| `[セット名]` | questions.json の `answerSets` から該当セットのメンバーをランダム選出（プレイ中のモードの世代で絞る。後述） |
| `[1期生]` / `[2期生]` | 指定世代のタレント名からランダム選出 |
| `[全員]` | 全タレント（1期生+2期生）からランダム選出 |
| `[寮名]`（例: `[ミュゥ寮]`） | 問題のgenerationに従い、該当寮のタレント名からランダム選出 |
| `[寮名+世代]`（例: `[バゥ寮1期生]`） | generationを無視し、指定世代の該当寮タレントからランダム選出 |
| `[寮名+全員]`（例: `[クゥ寮全員]`） | generationを無視し、全世代の該当寮タレントからランダム選出 |

**ゲーム側の補完ロジック（`text-quiz/generator.ts` の `fillAnswers`）:**
1. 各選択肢を順にチェック
2. `[セット名]` 形式 → 以下の優先順で解決:
   a. `answerSets` に該当セットがあれば、そのメンバーを**選択肢スコープ**（後述）で絞り込んでランダム選出
   b. `[1期生]` / `[2期生]` / `[全員]` → 指定世代（または全員）のタレントからランダム選出
   c. `[寮名]` → 問題のgenerationに従い該当寮のタレントからランダム選出
   d. `[寮名+世代]` / `[寮名+全員]` → generationを無視し指定条件の寮タレントからランダム選出
3. 空文字列 → 問題の世代でフィルタしたタレントプールから未使用の名前をランダム選出
4. 正解や既に使用した名前とは重複しない
5. 補完後にシャッフルして表示

**選択肢スコープ（`answerSets` の世代絞り込み）:**

`answerSets` のセットは世代混在で定義できる（例: 投票イベントの突破者リストは1期生・2期生の両方を含む）。世代混在セットをそのまま使うと、2期生モードのプレイ中に1期生の名前が選択肢に並んでしまう。これを防ぐため、`generateTextQuizQuestions` は呼び出し元から**選択肢スコープ**を受け取り、`answerSets` のメンバーをタレントの世代で絞り込む。

| 呼び出し元 | スコープ | 挙動 |
|-----------|---------|------|
| 知識クイズ（1期生モード） | `1` | セットの1期生メンバーのみ選択肢に出る |
| 知識クイズ（2期生モード） | `2` | セットの2期生メンバーのみ選択肢に出る |
| タイムアタック | `0` | セットの全メンバー（1期生+2期生）が選択肢に出る |

- 絞り込みは `answerSets` の `[セット名]` 解決のみに適用する。空文字列補完・`[1期生]`等の世代指定・寮指定は**問題の generation** に従う（従来動作を維持）
- talents.json に存在しないメンバー（外部ゲスト等。例: 声劇サークルの他校メンバー）は世代を判定できないため、スコープに関わらず常に候補に残す
- 絞り込み後の候補が0件になる場合は、絞り込みなしの候補にフォールバックする（セットが片方の世代のみで構成されている場合に選択肢が壊れないようにする安全策）
