...
export function getTimeMessage(elapsedMs: number): string {
  ...
  if (minutes < 10) return '驚異的なタイム！パレ学の神！'
  if (minutes < 15) return 'すごい！パレ学マスターの実力！'
  if (minutes < 20) return '素晴らしい！かなりの腕前！'
  if (minutes < 30) return 'いい感じ！まだまだ伸びしろあり！'
  if (minutes < 45) return 'クリアおめでとう！'
