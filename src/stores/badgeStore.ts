...
/** 指定エリアの全スロットが最大レベル（ゴールド）に達しているか。称号・解放判定の核。 */
export function isAreaComplete(badges: BadgeMap, slotIds: readonly BadgeSlotId[]): boolean {
...
export const useBadgeStore = create<BadgeState & BadgeActions>()((set, get) => ({
  ...
  // 称号・解放は現在のバッジから動的判定する（全スロット最大レベル = ゴールド）。
  // 過去にシルバー止まりだった到達者は救済しない（むずかしいで取り直す）。永続フラグは持たない。
